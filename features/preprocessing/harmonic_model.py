...
def _getFundamentalFrequency(frequencies: np.array, harmonicNums: np.array, axis: int) -> np.array:
    ...
    n  = (~np.isnan(frequencies)).sum(axis = axis, keepdims = True) # Num. points (i.e. peaks)
    x  = np.nansum(harmonicNums, axis = axis, keepdims = True)                # i.e.: x
    y  = np.nansum(frequencies, axis = axis, keepdims = True)                 # i.e.: y
    xy = np.nansum(harmonicNums * frequencies, axis = axis, keepdims = True)  # i.e.: x * y
    xx = np.nansum(harmonicNums * harmonicNums, axis = axis, keepdims = True) # i.e.: x * x
    f  = (n * xy - x * y) / (n * xx - x * x)            # Fitted slope = fundamental frequency
    f[n == 1] = y[n == 1] # If a single peak was found, it corresponds to the fundamental frequency
...
def harmonicModel(
    ...
    peakFrequencies: np.array, numHarmonics: int, 
