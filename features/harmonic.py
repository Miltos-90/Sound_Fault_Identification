...
def _truncate(x1: np.array, y1: np.array, x2: np.array, y2: np.array, axis: int
    ...
    if n2 < n1: # Pair 2 has fewer elements than pair 1 along axis <axis>
        ix       = np.arange(n2)
        x1_, y1_ = np.take(x1, ix, axis), np.take(y1, ix, axis)
        x2_, y2_ = x2, y2
    elif n1 < n2: # Pair 1 has fewer elements than pair 2
        ix       = np.arange(n1)
        x2_, y2_ = np.take(x2, ix, axis), np.take(y2, ix, axis)
        x1_, y1_ = x1, y1
...
def _inharmonicity(
    ...
    fundamentalFrequencies = np.take(harmonicFrequencies, [0], axis = axis)
...
def _oddToEvenEnergyRatio(amplitudes: np.array, axis: int) -> np.array:
    ...
    oddHarms  = np.take(harmAmps2, np.arange(1, numSample, 2), axis)
    evenHarms = np.take(harmAmps2, np.arange(0, numSample - 1, 2), axis)
...
def _tristimulus(amplitudes: np.array, axis: int) -> np.array:
    ...
    out = np.stack([
        np.take(amplitudes, 1, axis) / s,                                             # t1
        np.take(amplitudes, np.arange(2, 5), axis).sum(axis) / s,                     # t2
        np.take(amplitudes, np.arange(5, amplitudes.shape[axis]), axis).sum(axis) / s # t3
    ], axis = axis)
...
def _deviation(
    ...
    f0   = np.take(harmonicFrequencies, [0], axis = axis) # Extract fundamental frequency
